...
## 📋︎ Announcement 2026-08-31

This update releases the All-time Sequences data and its accompanying documentation.

- Added submittable-date sequences for all DQ levels
- Added the All-time Sequence Guide in English and Chinese
- Added documentation for the `Code` and `Date` data columns
- Updated the historical DQ date records through 2026-08-31 and restored the missing record for 2026-02-06
- Updated the `AGENTS.md` project collaboration guide

The submittable-date sequences were generated by SeqUniverse by searching within the minimum positive period of the date-seed cycle. They are intended to identify high-scoring Bonus sequences for each DQ level across the entire range of submittable dates. The data are stored in the `/data/Alltime` directory and are available in both English and Chinese. **Submittable-date sequences should be used in dev mode only. Do not use these dates to submit leaderboard scores or otherwise affect the current DQ leaderboard.**

The project has also added `AGENTS.md` to help AI agents and automated tools quickly understand the project structure, DQ lookup workflow, data file formats, sequence interpretation rules, project terminology, and compliance requirements. This document standardizes how automated tools handle the project and helps prevent incorrect derivation of the current DQ, sequence misinterpretation, or unintended changes to protected data files.
