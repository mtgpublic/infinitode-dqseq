...
本次更新发布了可提交日期序列（All-time Sequences）数据及其配套文档。

- 新增全部 DQ 关卡的可提交日期序列
- 新增中英文可提交日期序列指南
- 新增 `Code` 和 `Date` 数据列说明
- 将历史 DQ 日期记录更新至 2026-08-31，并补录 2026-02-06 的记录
- 更新 `AGENTS.md` 项目协作指南

可提交日期序列由 SeqUniverse 在日期种子的最小正周期内搜索生成，用于寻找各个 DQ 关卡在全部可提交日期范围内的高分奖励序列。数据存放于 `/data/Alltime` 目录，并同时提供中文和英文版本。**可提交日期序列应仅在开发者模式下使用。请勿使用这些日期向排行榜提交成绩，也不得以任何方式影响当前 DQ 排行榜**。

此外项目添加了 `AGENTS.md`，用于帮助 AI Agent 和自动化工具快速理解项目结构、DQ 查询流程、数据文件格式、序列解读规则、专有名词及合规要求。该文档能够统一自动化工具的处理方式，避免错误推算当前 DQ、误读序列或修改不应改动的数据文件。

*从 2025-12-31 的 dqseq 更新开始，所有的 dqseq 更新公告将频繁使用序列的缩写，请留意*。

[往期公告](https://mtgpublic.github.io/infinitode-dqseq/guide/Announcement.zh_CN)

每次公告都会同步更新 [历史 DQ 日期统计](https://mtgpublic.github.io/infinitode-dqseq/guide/Dailyquestsheet.zh_CN)，你可以点击查看。
...
当前仓库的数据使用 SeqUniverse-dqseq-20260718 进行生成。
