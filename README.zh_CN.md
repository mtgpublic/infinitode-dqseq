...
在以往的 DQ9 模型中，对甲分数和闪电球分数是评分的重要组成部分，其与伤害分数和易伤层级的相关度较低。而在最新的模型中，修正了伤害（伤害，攻速，易伤，暴击，全技能）分数和易伤层级对对甲分数和闪电球分数的影响。在伤害分数和易伤层级较低的情况下，对甲分数和闪电球分数将受到一定比例的扣减，最低为原本的 1%。

感谢 MTG 成员 Yevech 对此更新的贡献。
...
当前仓库的数据使用 SeqUniverse-dqseq-20260426 进行生成。
