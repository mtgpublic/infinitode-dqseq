...
- DQ4 与 DQ7 的易伤、DQ4 的矿等相关奖励分数调整

在当前环境下，易伤的实际收益比评分模型此前的预期要高，即 Debuff 后期平均覆盖数量更高。DQ4 与 DQ7 中每层易伤的评分由 25% 上调至 27.5%（即平均 5.5 的 Debuff 覆盖数）。

另一边，DQ4 的矿等权重相对过高，由 22%/24%/26% 下调至 20%/22%/24%。

需要注意的是，DQ4 的技能与随机技能仍然存在分数不准的问题，相关奖励的评分仍需要进一步讨论。

感谢 MTG 成员 Min0913 对此更新的贡献。
...
当前仓库的数据使用 SeqUniverse-dqseq-20260718 进行生成；其中 DQ4 与 DQ7 的数据已由 SeqUniverse-dqseq-20260909 重新生成（数据版本号 20260909）。
